...
public:
	VariableInfo(){}
	VariableInfo(std::string newname, UINT newpos, TypeInfo* newtype, UINT newcount=1, bool newisConst=true, bool newisRef=false):
	  name(newname), pos(newpos), varType(newtype), count(newcount), isConst(newisConst), isRef(newisRef){}
	std::string	name;		//Variable name
	UINT		pos;		//Variable position in value stack
	UINT		count;		//Element count
	bool		isConst;	//Constant flag
	bool		isRef;		//Reference flag
